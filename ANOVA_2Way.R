##################################################################
## One-way and Two-Way ANOVA wild & residual bootstrap          ##
##   formula:  response~factor1*factor2 or response~factor1     ##
##   B:  number of bootstrap re-samples                         ##
##   type:  type of bootstrap, choose "residual" or "wild"      ##
##   wild.dist:  type of wild bootstrap distn, choose "normal"  ##
##               "uniform", "lognorm", ... (others pre-coded)   ##
##   seed:  set the seed for simulation if you want to replicate##
##   data:  name of dataset variables are in                    ##
##   keep.boot.resp:  boolean indicating if bootstrap response  ##
##                    should be kept (can be very large)        ##
##################################################################

##update case in wild bootstrap distribution choices.  

anova.boot <- function(formula, B=1000, type="residual", wild.dist="normal",  
  ...
  if( !(wild.dist %in% c("normal", "uniform", "lognorm")) ){
    stop("Incorrect distribution specified for the wild bootstrap.\n")
  }
    ...
    for(j in 1:B){
      if (type == "residual") {
        ...
        if (wild.dist == "normal") {
          ...
          bootResid <- matrix(testResid * runif(n, -sqrt(3), sqrt(3)), ncol=1)  
        } else if (wild.dist == "lognorm") {
          bootResid <- matrix(testResid * rlnorm(n, meanlog=0, sdlog=sqrt(log(1/2 + sqrt(1+4)/2))) - 
                               exp(log(1/2 + sqrt(1+4)/2)/2), ncol=1)  
        } 
        yb <- matrix(y.hatnull + bootResid, nrow=n, ncol=1)      #bootstrap response under H0
      }
